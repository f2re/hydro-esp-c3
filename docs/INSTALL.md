...
Обычный `install` выполняет **чистую установку**: сначала проверяет сборку, затем очищает только сохранённые настройки контроллера (NVS) и прошивает устройство. Вся flash целиком не стирается. После успешной установки плата предсказуемо создаёт `HydroESP-Setup`, а не пытается уйти в старую Wi‑Fi сеть.
...
Никаких setup key, отдельных логинов или паролей сети настройки нет.
