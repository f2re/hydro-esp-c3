#!/usr/bin/env python3
"""One-shot idempotent normalizer for the embedded Web OTA block."""
...
start = "const otaSleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));"
end = "\n\ntry{const saved="
start_pos = text.find(start)
end_pos = text.find(end, start_pos)
if start_pos < 0 or end_pos < 0:
    raise SystemExit("Web OTA block boundaries not found")

canonical = r'''const otaSleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));
...
async function waitForOtaReboot(before,timeout=45000){const deadline=Date.now()+timeout,beforeUptime=Number(before?.uptime),beforeBuild=String(before?.build||'');let sawOffline=false;while(Date.now()<deadline){const current=await otaStatus();if(!current){sawOffline=true;await otaSleep(700);continue}const uptime=Number(current.uptime),build=String(current.build||''),uptimeReset=Number.isFinite(beforeUptime)&&Number.isFinite(uptime)&&uptime<beforeUptime,buildChanged=!!beforeBuild&&!!build&&build!==beforeBuild;if(!before||sawOffline||uptimeReset||buildChanged)return current;await otaSleep(700)}return null}
...
new_text = text[:start_pos] + canonical + text[end_pos:]
if new_text == text:
    print("Web OTA block already canonical")
else:
    path.write_text(new_text, encoding="utf-8")
    print("normalized Web OTA block")
