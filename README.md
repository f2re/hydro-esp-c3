...
**Подключил USB → запустил установщик → открыл адрес с OLED. Всё.**
...
На **новой или ещё не настроенной** плате после прошивки появляется открытая локальная сеть:
...
Если плата уже была настроена раньше, `install` **не стирает её данные**: она снова подключится к сохранённому Wi‑Fi, а прямой адрес сайта будет показан на OLED.

> [!IMPORTANT]
> Никаких setup-key, паролей установочной сети, логинов, erase-flash и мастеров безопасности в обычной установке нет. Установщик просто собирает и прошивает firmware.
...
|---|---|---|
| Установить / переустановить по USB | `bash install.sh` / `.\install.ps1` | сохраняются, если уже были |
...
`hydro.local` — только удобное имя и зависит от mDNS вашей сети. На новой ненастроенной плате setup-адрес — `http://192.168.4.1`.

## 🔓 Сеть без лишних барьеров

`HydroESP-Setup` намеренно открытая сеть для первичной локальной настройки. Никаких device key, логинов или дополнительных паролей установки нет. После сохранения домашнего Wi‑Fi контроллер перестаёт работать в setup AP и переходит в вашу LAN.
